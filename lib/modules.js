...
var _ramdaPath = _path.default.dirname(_module.default._resolveFilename('ramda-adjunct', (0, _ramda.merge)(new _module.default(), {
  'paths': _module.default._nodeModulePaths(process.cwd())
}))); // ramda folder will be /nodemodules/ramda/dist. We want to remove the dist


var ramdaPath = _ramdaPath.slice(0, _ramdaPath.lastIndexOf('ramda-adjunct') + 13); // We do not need to change the search path based on useES since src and es are both built from the
...
function resolveModule(useES, name) {
  for (var category in methods) {
    if ((0, _ramda.contains)(name, methods)) {
      return "ramda-adjunct/".concat(useES ? 'es' : 'lib', "/").concat(name);
    }
  }

  throw new Error("ramda-adjunct method ".concat(name, " was not a known function\n    Please file a bug if it's my fault https://github.com/megawac/babel-plugin-ramda/issues\n  "));
}
